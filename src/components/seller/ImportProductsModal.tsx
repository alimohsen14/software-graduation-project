...
export default function ImportProductsModal({ isOpen, onClose, onSuccess, importProductsFn }: ImportProductsModalProps) {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!selectedFile.name.endsWith('.xlsx')) {
            toast.error(t("seller.importModal.invalidFileType"));
            setFile(null);
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            toast.success(t("seller.importModal.importSuccess", { count: res.count }));
            onSuccess();
            onClose();
        } catch (err: any) {
            console.error("Import failed", err);
            const errorMessage = err.response?.data?.message || t("seller.importModal.importFailed");
            ...
            <div className={`bg-zinc-950/40 backdrop-blur-2xl rounded-[3rem] border border-white/10 w-full max-w-lg shadow-2xl shadow-black/50 overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col relative ${isAr ? "text-right" : "text-left"}`}>
                ...
                <div className={`px-6 md:px-10 py-6 md:py-8 bg-white/5 border-b border-white/5 flex items-center justify-between z-10 shrink-0 ${isAr ? "flex-row-reverse" : ""}`}>
                    <div className="flex-1">
                        <h3 className="text-xl md:text-2xl font-black text-white tracking-tight uppercase leading-none">{t("seller.importModal.title")}</h3>
                        <p className="text-[9px] text-white/30 font-bold uppercase tracking-widest mt-2">{t("seller.importModal.subtitle")}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 md:p-10 space-y-8 md:space-y-10 relative z-10">
                    ...
                    <div className="bg-emerald-500/10 rounded-2xl p-5 md:p-6 border border-emerald-500/20 relative overflow-hidden group">
                        ...
                        <div className={`flex gap-4 relative z-10 ${isAr ? "flex-row-reverse text-right" : "text-left"}`}>
                            ...
                            <div>
                                <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1.5">{t("seller.importModal.instructionsTitle")}</p>
                                <p className="text-[11px] text-white/40 leading-relaxed font-bold uppercase tracking-widest">
                                    {t("seller.importModal.instructionsDesc")}<br />
                                ...
                                <p className="text-[9px] text-white/20 mt-3 font-black uppercase tracking-widest italic">
                                    {t("seller.importModal.optionalColumns")}
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        {file ? (
                            <div className="space-y-4 animate-in fade-in zoom-in duration-500 relative z-10">
                                ...
                                <div>
                                    ...
                                    <p className="text-[9px] text-white/30 font-bold uppercase tracking-widest">{(file.size / 1024).toFixed(1)} KB • {t("seller.importModal.readyToUpload")}</p>
                                </div>
                                <button
                                ...
                                >
                                    {t("seller.importModal.cancelFile")}
                                </button>
                            </div>
                        ) : (
                            <div className="relative z-10">
                                ...
                                <p className="text-[11px] md:text-xs font-black text-white uppercase tracking-widest">{t("seller.importModal.dropzonePlaceholder")}</p>
                                <p className="text-[9px] text-white/20 mt-2 font-black uppercase tracking-widest">{t("seller.importModal.formatRequired")}</p>
                            </div>
                        )}
                    </div>
                        ...
                        >
                            {uploading ? (
                                <>
                                    ...
                                    {t("seller.importModal.importing")}
                                </>
                            ) : (
                                t("seller.importModal.startImport")
                            )}
                        </button>
                        <button
                            onClick={onClose}
