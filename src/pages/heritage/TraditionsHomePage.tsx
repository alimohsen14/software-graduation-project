...
export default function TraditionsHomePage() {
    const navigate = useNavigate();
    const { t, i18n } = useTranslation("traditions");
    const [currentIndex, setCurrentIndex] = useState(0);

    const nextSlide = () => {
        setCurrentIndex((prev) => (prev + 1) % traditionsData.length);
    };

    const prevSlide = () => {
        setCurrentIndex((prev) => (prev - 1 + traditionsData.length) % traditionsData.length);
    };

    const handleCardClick = (id: string) => {
        if (id === "wedding") {
            navigate("/heritage/traditions/wedding");
        } else if (id === "mourning") {
            navigate("/traditions/condolence");
        } else if (id === "palestinian-food") {
            navigate("/heritage/foods");
        } else if (id === "clothes") {
            navigate("/heritage/customs/clothes");
        } else {
            alert("قريباً...");
        }
    };
            ...
            >
                ...
                <section className="relative w-full max-w-[900px] flex items-center justify-center px-4 md:px-12">
                    ...
                    <div className="relative w-full flex md:block overflow-x-auto md:overflow-visible snap-x snap-mandatory no-scrollbar gap-4 py-8 md:h-[450px]">
                        {traditionsData.map((item, index: number) => (
                            <LibraryCard
                                key={item.id}
                ...
                <div className="flex gap-2 mt-4">
                    {traditionsData.map((_: any, index: number) => (
                        <div
                            key={index}
