...
export default function MarketplacePage() {
    ...
    return (
        <DashboardLayout>
            <div dir={direction} className="relative min-h-screen">
                {/* 1. Hero Section */}
                <section className="relative pt-12 md:pt-24 pb-8 md:pb-16 px-6 sm:px-10 overflow-hidden">
                    {/* Dark gradient overlay for readability */}
                    <div className={`absolute inset-0 z-0 bg-gradient-to-${direction === 'rtl' ? 'l' : 'r'} from-black/60 via-black/20 to-transparent pointer-events-none`} />

                    <div className={`max-w-7xl mx-auto relative z-10 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
                        <motion.div
                        ...
                        <div className={`flex flex-wrap items-center ${direction === 'rtl' ? 'justify-start' : 'justify-start'} gap-4 mt-8`}>
                            {showBecomeSeller && (
                                <button
                            ...
                            {(isSeller || !!user?.isAdmin) && (
                                ...
                                >
                                    {user?.isAdmin ? t("dashboard") : t("sellerAccount")}
                                </button>
                            )}
                ...
                <div className="w-full px-4 md:px-8 py-8">
                    <div className="max-w-7xl mx-auto" dir={direction}>
                        {isPending && (
                            <div className="mb-8 bg-amber-500/10 border border-amber-500/20 text-amber-400 px-6 py-4 rounded-3xl flex items-center gap-4 backdrop-blur-md">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-[9px] font-black uppercase tracking-widest opacity-50 mb-0.5">{t("status")}: {t("translation:auth.pending")}</span>
                        ...
                        {loading && products.length === 0 ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-32 bg-white/5 backdrop-blur-md rounded-[2.5rem] border border-white/10 shadow-2xl animate-in zoom-in duration-500">
                                ...
                                <p className="text-xl font-black text-white uppercase tracking-tighter mb-1">
                                    {isSearching ? t("searching") : t("empty.title")}
                                </p>
                                <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">
                                    {isSearching ? t("searchingDesc") + searchTerm : t("empty.desc")}
                                </p>
                            </div>
                        ) : (
                            <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 transition-opacity duration-300 ${loading ? "opacity-40" : "opacity-100"}`}>
                ...
                <StoreSocialLists
                    ...
                    title={socialModalType === 'followed' ? t("followedStores") : t("favoriteStores")}
                    stores={socialStores}
