...
export default function StorePage() {
    ...
    return (
        <DashboardLayout>
            <div dir={direction} className="w-full min-h-screen p-6 sm:p-12 lg:p-20 animate-in fade-in duration-700">
                <div className="max-w-7xl mx-auto">
                    ...
                    >
                        <FiArrowLeft size={16} className={`${direction === 'rtl' ? 'group-hover:translate-x-1' : 'group-hover:-translate-x-1'} transition-transform`} />
                        {t("store.backToMarketplace")}
                    </button>

                    {/* Store Branding Header */}
                    <div className="bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-2xl p-6 md:p-8 mb-12 overflow-hidden relative group">
                        ...
                        <div className="flex flex-col lg:flex-row items-center lg:items-start gap-6 relative z-10">
                            {/* Logo Wrapper */}
                            <div className="w-24 h-24 md:w-28 md:h-28 bg-zinc-900 rounded-2xl flex items-center justify-center overflow-hidden border border-white/10 shadow-2xl transition-transform duration-700 group-hover:scale-105 shrink-0 relative">
                                {store?.logo ? (
                                    <img src={store.logo} alt={store.name} className="w-full h-full object-cover" />
                                ) : (
                                    <FiShoppingBag className="w-10 h-10 md:w-12 md:h-12 text-white/10" />
                                )}
                            ...
                            <div className="flex-1 w-full text-center lg:text-left">
                                <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-4">
                                    <div>
                                        <div className="flex items-center justify-center lg:justify-start flex-wrap gap-3 mb-3">
                                            <h1 className="text-3xl md:text-4xl font-black text-white tracking-tighter uppercase leading-none">{store?.name}</h1>
                                            {store?.isOfficial && (
                                                <div className="bg-indigo-600/20 text-indigo-400 border border-indigo-500/20 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 shadow-lg shadow-indigo-500/5">
                                                    <FiShield size={12} />
                                        ...
                                        {store?.ownerName && (
                                            <div className="flex items-center justify-center lg:justify-start gap-3">
                                                <div className="w-6 h-6 bg-white/5 rounded-full flex items-center justify-center border border-white/5 shadow-inner">
                                                    <FiUser size={12} className="text-emerald-500" />
                                                </div>
                                                <span className="text-[10px] font-black uppercase tracking-widest text-white/30">{t("store.storeManager")} <span className="text-white">{store.ownerName}</span></span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex items-center justify-center gap-3">
                                        <button
                                            ...
                                            className={`w-11 h-11 rounded-xl border transition-all duration-300 flex items-center justify-center shadow-xl ${isFavorited
                                            ...
                                            title={isFavorited ? t("store.removePriority") : t("store.markPriority")}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all duration-300 shadow-2xl border ${isFollowed
                                        ...
                                        >
                                            {isFollowed ? (
                                                <><FiCheck size={16} /> {t("store.established")}</>
                                            ) : (
                                                <><FiPlus size={16} /> {t("store.followStore")}</>
                                            )}
                                        </button>
                                    </div>
                                </div>
                                ...
                                {store?.description ? (
                                    <p className="text-white/40 leading-relaxed max-w-3xl mx-auto lg:mx-0 text-sm md:text-base font-medium">
                                        {store.description}
                                    </p>
                                ) : (
                                    <p className="text-white/20 uppercase tracking-widest text-xs italic">{t("store.storeDescription")}</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-8 relative z-10">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <h2 className="text-3xl font-black text-white tracking-tighter uppercase leading-none mb-2">{t("availableProducts")}</h2>
                                <p className="text-white/30 text-[10px] font-black uppercase tracking-[0.2em] leading-none">{t("browseProducts")}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {loading ? (
                        <div className="min-h-[400px] flex flex-col items-center justify-center animate-pulse">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-white/20">{t("syncingCatalog")}</span>
                        </div>
                    ) : products.length === 0 ? (
                        <div className="bg-white/5 backdrop-blur-md rounded-[3rem] border border-white/10 flex flex-col items-center justify-center py-32 px-10 text-center animate-in zoom-in duration-500 shadow-2xl">
                            ...
                            <h3 className="text-2xl font-black text-white uppercase tracking-tighter mb-2">{t("nullManifest")}</h3>
                            <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">{t("noUnitsAvailable")}</p>
                        </div>
                    ) : (
                        <>
